...
test("update script reuses the saved credential and reinstalls without prompting", { skip: process.platform === "win32" }, async () => {
  const token = "installer-client-token-1234567890";
  let verified = false;
  const server = createServer((request, response) => {
    verified = request.url === "/api/client/verify" && request.headers.authorization === `Bearer ${token}`;
  ...
  const root = await mkdtemp(path.join(os.tmpdir(), "solreviewupdatetest"));
  const installRoot = path.join(root, "install");
  const skillsRoot = path.join(root, "skills");
  const address = server.address();
  const url = `http://127.0.0.1:${address.port}`;
  ...
  try {
    // The state an installed machine has: a saved credential, and stale files about to be replaced.
    await mkdir(path.join(installRoot, "client"), { recursive: true });
    await writeFile(path.join(installRoot, "remote.json"), JSON.stringify({ url, token }));
    await writeFile(path.join(installRoot, "client", "solreview.js"), "stale\n");

    const result = await new Promise((resolve, reject) => {
      const child = spawn("sh", [path.resolve("update.sh")], {
        stdio: ["ignore", "pipe", "pipe"],
        env: {
          ...process.env,
          HOME: root,
    ...
    assert.equal(verified, true);
    assert.doesNotMatch(`${result.stdout}${result.stderr}`, new RegExp(token));
    assert.match(result.stdout, /Updating Sol Review Gate/);
    const client = await readFile(path.join(installRoot, "client", "solreview.js"), "utf8");
    assert.match(client, /const TERMINAL/);
    assert.doesNotMatch(client, /^stale$/m);
    assert.match(await readFile(path.join(skillsRoot, "sol", "SKILL.md"), "utf8"), /^name: sol$/m);
    assert.match(await readFile(path.join(skillsRoot, "solute", "SKILL.md"), "utf8"), /^name: solute$/m);
  } finally {
    await new Promise((resolve) => server.close(resolve));
...
test("update script refuses to run without an existing installation", { skip: process.platform === "win32" }, async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "solreviewupdatetest"));
  const result = await new Promise((resolve, reject) => {
    const child = spawn("sh", [path.resolve("update.sh")], {
      stdio: ["ignore", "pipe", "pipe"],
      env: { ...process.env, HOME: root, SOL_INSTALL_ROOT: path.join(root, "missing") },
    });
    let stderr = "";
    child.stderr.on("data", (chunk) => { stderr += chunk; });
    child.on("error", reject);
  ...
  assert.equal(result.code, 1);
  assert.match(result.stderr, /Run the installer once first/);
});
