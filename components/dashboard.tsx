...
import { fileEvidence, researchStatus, type FileEvidence, type ResearchTrace as ResearchTraceRecord } from "@/lib/research";
...
function FileEvidenceCard({ files }: { files: FileEvidence }) {
  if (!files.attached) return null;
  // "Attached" is what the reviewer could read; "referenced" is what it actually used. The gap is
  // the interesting number, because an attachment nothing refers to did no work.
  const named = files.cited === files.attached
  ...
  return <section className={`file-evidence file-${files.level}`}>
    ...
    {files.uncitedPaths.length > 0 && <><h3>Attached, not referenced</h3><ul className="muted">{files.uncitedPaths.map((file) => <li key={file}><code>{file}</code></li>)}</ul></>}
  </section>;
}
